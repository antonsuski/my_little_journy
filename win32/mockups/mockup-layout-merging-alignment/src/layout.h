...
/**
 * @brief Types of alignment and behaviour of window inside mesh.
 * 
 */
typedef enum
{
    NO_RESIZE = 1, /*< Forbids window sizing with element's mesh size*/
} alignmnet_t;
...
typedef struct
{
    ...
    alignmnet_t         alignment;                  /*< Type of alignment */
 ...
 * @param[in] align Set window alingment or behaviour.
 * @return BOOL Returns TRUE if addition is seccessfull, returns FALSE if failed
 */
BOOL add_control_into_mesh(layout_mesh_t* mesh, HWND hwnd, alignmnet_t align);
